            ...
            listConf: {
                ...
                columns: [
                    ...
                    { title: '考核地区', align: 'center', key: 'region_name', width: 200, render: (h, params) => {
                        if (params.row.kpiRule) {
                            var region_name = ''
                            var first = true
                            for (let region of params.row.kpiRule.region_name_arr) {
                                if (first) {
                                    region_name += region
                                } else {
                                    region_name += '-' + region
                    ...
                    { title: '开始时间', align: 'center', key: 'check_start_at_date', width: 120 },
                    { title: '结束时间', align: 'center', key: 'check_end_at_date', width: 120 },
                    { title: '考核周期', align: 'center', key: 'cycle_name', width: 90, render: (h, params) => {
                        return params.row.kpiRule ? h('span', params.row.kpiRule.cycle_name) : h('span', '')
                    }},
                    { title: '考核类型', align: 'center', key: 'type_name', width: 90, render: (h, params) => {
                        return params.row.kpiRule ? h('span', params.row.kpiRule.type_name) : h('span', '')
                    }},
                    { title: '考核标准', align: 'center', key: 'standard', width: 90, render: (h, params) => {
                        return params.row.kpiRule ? h('span', params.row.kpiRule.standard) : h('span', '')
                    }},
                    { title: '完成标准', align: 'center', key: 'real_standard', width: 90},
                    { title: '状态', align: 'center', key: 'finish_name', width: 100 },
